"""
Facebook Reels Automation - Bilingual English/Polish Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Polish branding
"""
...
# Polish translations for display
CATEGORIES_POLISH = {
    ...
    "Wisdom": "Mądrość",
    "Happiness": "Szczęście",
    "Self Improvement": "Rozwój Osobisty",
...
POLISH_VOICE = "pl-PL-MarekNeural"
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 2} unique {category_english} phrases for English speakers learning Polish.
...
2. Polish translation (with commas matching the rhythm)
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "polish": "Wierz w siebie.", "pronunciation": "Vyezh v seh-bye."},
            {"english": "You are capable of amazing things.", "polish": "Jesteś zdolny do niesamowitych rzeczy.", "pronunciation": "Yes-tesh z-dol-ny do nye-sa-mo-vi-tykh zhe-chy."},
            {"english": "Dream big, start small.", "polish": "Marz wielko, zaczynaj od małych kroków.", "pronunciation": "Mazh vyel-ko, za-chi-nay od ma-wykh kro-koof."},
            {"english": "Your future is created by your actions.", "polish": "Twoja przyszłość jest tworzona przez twoje działania.", "pronunciation": "Tvo-ya pshi-shwoshch yest tvo-zho-na pshes tvo-ye dzia-wa-nya."},
            {"english": "Never give up on your dreams.", "polish": "Nigdy nie rezygnuj ze swoich marzeń.", "pronunciation": "Nig-dy nye re-zig-nooy ze svo-ikh ma-zhen."},
        ],
        "Love": [
            {"english": "Love yourself first.", "polish": "Kochaj siebie przede wszystkim.", "pronunciation": "Ko-khay seh-bye pshede fshyst-keem."},
            {"english": "Love makes everything possible.", "polish": "Miłość czyni wszystko możliwym.", "pronunciation": "Mi-woshch chi-ni fshyst-ko mozh-li-vym."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        pl_success = asyncio.run(generate_single_audio(phrase["polish"], POLISH_VOICE, str(polish_file)))
        if pl_success:
            print(f"    ✓ Polish: {polish_file.name}")
        else:
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "2", str(polish_file)]
            subprocess.run(cmd, capture_output=True)

        # Get ACTUAL durations
        en_duration = get_audio_duration(str(english_file))
        pl_duration = get_audio_duration(str(polish_file))

        # Add pause between English and Polish
        pause_between = 0.5
        total_duration = en_duration + pause_between + pl_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + PL: {pl_duration:.2f}s)")
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{polish_file.as_posix()}'\n")
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    # Polish text
    polish_y = english_y + total_height + 110  # Increased from 100
    polish_lines = wrap_text(polish, font_large, VIDEO_WIDTH - 140)
    total_height = len(polish_lines) * 95  # Increased from 75

    draw.rectangle(
        [(60, polish_y - 55), (VIDEO_WIDTH - 60, polish_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(polish_lines):
        y_pos = polish_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = polish_y + total_height + 90  # Increased from 80
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_POLISH[category_english]})")
    print(f"{'='*80}\n")
    ...
    print(f"  🏷️  Branding: Velocity Polish")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇵🇱 VELOCITY POLISH - FACEBOOK REELS AUTOMATION 🇵🇱")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ Velocity Polish branding at bottom")
